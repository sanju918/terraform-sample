...
resource "azurerm_subnet" "web_server_subnet" {
  name = "${var.resource_prefix}-${substr(
    var.web_server_subnets[count.index],
    0,
    length(var.web_server_subnets[count.index]) - 3,
  )}-subnet"
  resource_group_name       = azurerm_resource_group.web_server_rg.name
...
# Public IP Address
resource "azurerm_public_ip" "web_server_lb_publicip" {
...
resource "azurerm_lb" "web_server_lb" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.web_server_lb_publicip.id
  }
}

resource "azurerm_lb_backend_address_pool" "web_server_backend_pool" {
  name                = "${var.resource_prefix}-lb_backend_pool"
  resource_group_name = azurerm_resource_group.web_server_rg.name
...
resource "azurerm_lb_rule" "web_server_lb_http_rule" {
  ...
  frontend_ip_configuration_name = "${var.resource_prefix}-lb_frontend-ip"
